...
export default function ClothingViewer() {
    ...
    // Add this state to track if we've already set random items
    const [randomItemsSet, setRandomItemsSet] = useState(false);
    ...
    useEffect(() => {
        ...
        setDynamicTextures({
            upper: {
                ...
                : null, // Will be filled with random item later
    ...
    // Add new useEffect to set random items once data is loaded - MOVED HERE AFTER OPTIONS ARE DEFINED
    useEffect(() => {
        // Only proceed if we have clothing options available AND haven't set random items yet
        if (upperClothingOptions.length > 0 && lowerClothingOptions.length > 0 && !randomItemsSet) {
            // Get random items
            const randomUpper = getRandomItem(upperClothingOptions);
            const randomLower = getRandomItem(lowerClothingOptions);
            
            setDynamicTextures(prevTextures => ({
                upper: {
                    // If user selected upper (isUpper is true), keep it; otherwise use random
                    url: isUpper 
                        ? prevTextures.upper.url 
                        : randomUpper.textureUrl,
                    name: isUpper 
                        ? prevTextures.upper.name 
                        : randomUpper.name,
                },
                lower: {
                    // If user selected lower (!isUpper is true), keep it; otherwise use random
                    url: !isUpper 
                        ? prevTextures.lower.url 
                        : randomLower.textureUrl,
                    name: !isUpper 
                        ? prevTextures.lower.name 
                        : randomLower.name,
                },
            }));
    ...
    }, [upperClothingOptions, lowerClothingOptions, isUpper, getRandomItem, randomItemsSet]);
