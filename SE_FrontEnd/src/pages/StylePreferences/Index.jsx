...
import {
  Box, Typography, Container, LinearProgress, Grid, Card, CardMedia,
  CardContent, CardActionArea, Button, Stack, Skeleton
} from "@mui/material";
...
export default function StylePreferences() {
        ...
        try {
          ...
          data.forEach(group => {
            ...
            if (group.category === "Old_Money") {
              raw = "https://swipe-fit.s3.ap-southeast-2.amazonaws.com/thumbnails/clothes/09a2bef1-f9d8-44fc-a082-d382e977c928.png";
            }
  ...
  const updateSession = (newData) => {
    ...
    sessionStorage.setItem("user-credentials", JSON.stringify({ ...current, ...newData }));
  };
        ...
        <Stack direction="row" spacing={2} justifyContent="center" mt={6}>
          ...
          <Button
            ...
            sx={{
              ...
              "&.Mui-disabled": { backgroundColor: "#c5cae9", color: "#9fa8da" }
            }}
          >
            Next
          </Button>
    ...
    return renderCommonLayout(
      staticStyleOptions.map(style => (
        <Grid item xs={12} sm={6} md={4} key={style.id}>
          ...
          >
            <CardActionArea>
              <CardMedia component="img" height="300" image={style.image} alt={style.name} sx={{ objectFit: "cover" }} />
              <CardContent sx={{ textAlign: "center", py: 2 }}>
                <Typography variant="h5" sx={{ fontWeight: 500 }}>{style.name}</Typography>
              </CardContent>
            </CardActionArea>
          </Card>
        </Grid>
